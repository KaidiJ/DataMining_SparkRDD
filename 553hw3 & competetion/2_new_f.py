...
def load_avg_ratings(sc, user_avg_path, business_avg_path):
    user_data = sc.textFile(user_avg_path).map(json.loads).map(lambda user: (user['user_id'], (
        user['average_stars'], user['review_count'], user['useful'] ** 2, user['funny'] ** 2, user['cool'] ** 2,
        user['fans'] ** 2,
        sum(x ** 2 for x in [
    ...
    business_data = sc.textFile(business_avg_path).map(json.loads).map(lambda business: (business['business_id'], (
        ...
        int(business['attributes']['RestaurantsPriceRange2']) ** 0.5 if business['attributes'] and 'RestaurantsPriceRange2' in business['attributes'] else 0
    )))
...
def feature_engineering(rdd, user_data, business_data, include_rating=True):
    if include_rating:
        ...
        final_rdd = train_rdd.leftOuterJoin(business_data).map(lambda x: (x[0], x[1][0][0], x[1][0][1], x[1][0][2][0], x[1][0][2][1], x[1][0][2][2], x[1][0][2][3], x[1][0][2][4], x[1][0][2][5], x[1][0][2][6], x[1][1][0] if x[1][1] else 0.0, x[1][1][1] if x[1][1] else 0, x[1][1][2] if x[1][1] else 0, x[1][1][3] if x[1][1] else 0))  # 使用默认值填充商家数据
    else:
        ...
        final_rdd = final_rdd.leftOuterJoin(business_data).map(lambda x: (x[0], x[1][0][0], x[1][0][1][0], x[1][0][1][1], x[1][0][1][2], x[1][0][1][3], x[1][0][1][4], x[1][0][1][5], x[1][0][1][6], x[1][1][0] if x[1][1] else 0.0, x[1][1][1] if x[1][1] else 0, x[1][1][2] if x[1][1] else 0, x[1][1][3] if x[1][1] else 0))  # 使用默认值填充商家数据
    return final_rdd
